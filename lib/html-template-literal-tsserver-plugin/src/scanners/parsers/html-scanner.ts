/**
 * This scanner has been forked from https://github.com/microsoft/vscode-html-languageservice/blob/main/src/parser/htmlScanner.ts
 * */
...
import type { TokenType as TokenTypeType, ScannerState as ScannerStateType, Scanner } from "vscode-html-languageservice";
const { TokenType, ScannerState } = pkg;
...
export function createScanner(input: string, initialOffset = 0, initialState: ScannerStateType = ScannerState.WithinContent, emitPseudoCloseTags = false): Scanner {
    ...
    function finishToken(offset: number, type: TokenTypeType, errorMessage?: string): TokenTypeType {
        tokenType = type;
        tokenOffset = offset;
    ...
    function internalScan(): TokenTypeType {
        const offset = stream.pos();
        if (stream.eos()) {
